...
const FuriganaText = ({ text }: { text: string }) => {
    ...
    return (
        <span>
            {parts.map((part, i) => {
                // Match [Kanji|Reading|Meaning]
                const match3 = part.match(/^\[([^\|]+)\|([^\|]+)\|([^\]]+)\]$/);
                if (match3) {
                    return (
                        <span key={i} className="inline-flex flex-col items-center align-middle mx-1 leading-none">
                            <span className="text-[10px] text-muted-foreground mb-0.5">{match3[2]}</span>
                            <span className="text-base font-medium">{match3[1]}</span>
                            <span className="text-[10px] text-muted-foreground mt-0.5">{match3[3]}</span>
                        </span>
                    );
                }
                ...
                if (match2) {
                    return (
                        <ruby key={i} className="mx-1">
                            {match2[1]}
                            <rt className="text-xs text-muted-foreground select-none">{match2[2]}</rt>
                        </ruby>
                    );
                }

                // Match [Text||Meaning] (No reading, just meaning below)
                const matchMeaning = part.match(/^\[([^\|]+)\|\|([^\]]+)\]$/);
                if (matchMeaning) {
                    return (
                        <span key={i} className="inline-flex flex-col items-center align-middle mx-1 leading-none">
                            {/* Spacer to align if needed, or just Text + Meaning */}
                            <span className="text-base font-medium">{matchMeaning[1]}</span>
                            <span className="text-[10px] text-muted-foreground mt-0.5">{matchMeaning[2]}</span>
                        </span>
                    );
                }
    ...
    return (
        <Card className={cn(
            ...
            <CardContent className="p-4 pt-3">
                <RadioGroup value={userAnswer} onValueChange={onSelect} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                ...
                {/* Explanation */}
                {showResult && (
                    <div className="mt-3 text-sm bg-white p-3 rounded border text-muted-foreground animate-in fade-in slide-in-from-top-2 whitespace-pre-wrap">
                        <span className="font-semibold text-foreground block mb-1">Giải thích:</span>
                        <FuriganaText text={q.explain} />
                    </div>
                )}
            </CardContent>
        </Card>
