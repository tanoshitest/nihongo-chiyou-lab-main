...
const VocabularyTab = ({ vocabulary }: { vocabulary: IrodoriVocabularyItem[] }) => {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-lg text-black">Từ vựng bài học ({vocabulary.length} từ)</CardTitle>
      </CardHeader>
          ...
          <TableBody>
            {vocabulary.map((item, index) => (
              <TableRow key={item.id}>
                ...
                <TableCell className="font-medium text-black">{item.kana}</TableCell>
          ...
          >
            <AccordionTrigger className="px-4 py-3 hover:no-underline hover:bg-muted/50">
              <div className="flex items-start gap-3 text-left">
                <span className="flex-shrink-0 flex items-center justify-center w-8 h-8 rounded-full bg-black text-white text-sm font-bold">
                  {index + 1}
                </span>
                <div>
                      ...
                      {module.content.vocabulary_focus.map((vocab, vIndex) => (
                        <div key={vIndex} className="p-3 bg-muted/50 rounded-lg space-y-2">
                          <div className="flex items-center gap-2">
                            <span className="font-medium text-black">{vocab.word}</span>
                          ...
                          {dialogue.lines.map((line, lIndex) => (
                            <div key={lIndex} className="flex gap-3 items-start">
                              ...
                              <div className="flex-1">
                                <p className="text-black font-medium">{line.japanese}</p>
                  ...
                  <div className="space-y-3">
                    {module.content.phonics_rules.map((rule, rIndex) => (
                      <div key={rIndex} className="border rounded-lg p-4">
                        <h5 className="font-medium text-black mb-2">🔊 {rule.rule}</h5>
                        ...
                        <div className="space-y-2">
                          {rule.examples.map((ex, eIndex) => (
                            <div key={eIndex} className="flex flex-wrap items-center gap-2 text-sm bg-muted/50 rounded p-2">
                              <span className="font-medium text-black">{ex.japanese}</span>
                ...
                {module.content.tips && module.content.tips.length > 0 && (
                  <div className="space-y-3">
                    {module.content.tips.map((tip, tIndex) => (
                      <div key={tIndex} className="border-l-4 border-black bg-black/5 rounded-r-lg p-4">
                        <h5 className="font-medium text-black mb-2">🎌 {tip.title}</h5>
        ...
        <Card className="border-black">
          <CardContent className="py-6">
            <div className="text-center space-y-2">
              ...
              <Button onClick={handleReset} variant="outline" className="mt-4 border-black text-black hover:bg-black/5">
      ...
      {quiz.map((question, qIndex) => {
        ...
        return (
          <Card
            ...
            className={cn(
              "transition-all",
              showResult && isCorrect && "border-black bg-black/5",
            ...
            <CardHeader className="pb-3">
              <CardTitle className="text-base flex items-start gap-2">
                <span className={cn(
                  ...
                  showResult && isCorrect ? "bg-black text-white" :
            ...
            <CardContent className="space-y-4">
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          isSubmitted && isThisCorrect && "border-black bg-black/10",
                      ...
                      >
                        <RadioGroupItem
                          ...
                          className={cn(
                            isSubmitted && isThisCorrect && "border-black text-black",
                        ...
                        <Label
                          ...
                          className={cn(
                            ...
                            isSubmitted && isThisCorrect && "text-black font-medium",
              ...
              {showResult && (
                <div className={cn(
                  ...
                  isCorrect ? "bg-black/5 border-black" : "bg-black/5 border-amber-500"
                )}>
      ...
      {!isSubmitted && (
        <div className="flex justify-center pt-4">
          <Button
            ...
            className="bg-black hover:bg-neutral-800 text-white px-8"
            size="lg"
          >
            Nộp bài
          </Button>
        ...
        <main className="container mx-auto px-4 py-6">
          <Tabs defaultValue="vocabulary" className="space-y-6">
            <TabsList className="grid w-full grid-cols-3 h-auto p-1">
              <TabsTrigger value="vocabulary" className="flex flex-col sm:flex-row items-center gap-1 py-2 data-[state=active]:bg-black data-[state=active]:text-white">
                <BookOpen className="w-4 h-4" />
                <span className="text-xs sm:text-sm">Từ vựng</span>
              </TabsTrigger>
              <TabsTrigger value="grammar" className="flex flex-col sm:flex-row items-center gap-1 py-2 data-[state=active]:bg-black data-[state=active]:text-white">
                <PenTool className="w-4 h-4" />
                <span className="text-xs sm:text-sm">Bài giảng</span>
              </TabsTrigger>
              <TabsTrigger value="quiz" className="flex flex-col sm:flex-row items-center gap-1 py-2 data-[state=active]:bg-black data-[state=active]:text-white">
