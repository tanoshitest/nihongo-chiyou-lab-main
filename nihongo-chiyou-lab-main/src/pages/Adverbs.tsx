...
import { ADVERBS } from "@/data/adverbsData";
    ...
    return (
        <span className="inline-flex flex-wrap items-center gap-y-10 leading-[4rem] text-lg text-gray-700 font-medium py-2">
            {parts.map((part, index) => {
                ...
                // Handle Highlight: {Text} (for Adverbs)
                if (part.startsWith('{') && part.endsWith('}')) {
                    const content = part.slice(1, -1); // Remove { }
                    return (
        ...
        <Layout>
            <div className="container mx-auto px-4 py-8 max-w-5xl">
                <div className="flex items-center gap-4 mb-8">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold text-[#008001]">Các cách nói dùng phó từ</h1>
                        <p className="text-muted-foreground mt-1">Tổng hợp phó từ quan trọng và ví dụ minh họa</p>
                    </div>
                </div>
                    ...
                    {ADVERBS.map((group) => (
                        <div key={group.id} className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 gap-8">
                                {group.items.map((adverb) => (
                                    <Card key={adverb.id} className="scroll-mt-20 shadow-sm hover:shadow-md transition-shadow" id={`adverb-${adverb.id}`}>
                                        <CardHeader className="border-b bg-slate-50/50 pb-4">
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-3">
                                                    <div className="flex flex-col">
                                                        <CardTitle className="text-xl font-bold flex items-center gap-3">
                                                            <span className="font-bold text-2xl text-[#008001]">{adverb.word}</span>
                                                            <span className="text-slate-500 font-normal text-lg">({adverb.romaji})</span>
                                                        </CardTitle>
